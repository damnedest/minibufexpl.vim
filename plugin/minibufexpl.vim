...
function! <SID>BuildBufferList(delBufNum, updateBufList)
  ...
  while(l:i < l:NBuffers)
    ...
    if(getbufvar(l:index, '&modified') == 1)
      let l:tab = l:tab . '+'
    endif
...
function! <SID>TrimFilePathes(filePathes)
  ...
  while(l:i < l:buffersCount - 1)
    ...
    while (l:j < l:buffersCount)
      ...
      let l:measureIndex = l:measureLength - 1

      " Find first different symbol from rigth at both path
      while(l:victimIndex > 0 &&
          \ l:measureIndex > 0 &&
          \ l:trimVictim[l:victimIndex] ==
          \ l:trimMeasure[l:measureIndex])
        let l:measureIndex = l:measureIndex - 1
        let l:victimIndex = l:victimIndex - 1
      endwhile

      " Check if current finded position expanded previous
      if (l:victimIndex < l:trimPositions[l:i])
        let l:trimPositions[l:i] = l:victimIndex
      endif
      if (l:measureIndex < l:trimPositions[l:j])
        let l:trimPositions[l:j] = l:measureIndex
      endif
  ...
  " Expand to right pathes to include all
  " symbols before first '\' or '/'.
  " And add result to output map.
  let l:i = 0
